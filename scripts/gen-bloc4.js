...
const sec3 = [
  ...
  tbl(['ID', 'Priorité', 'Module', 'Statut'], [
    ...
    ['BUG-07', 'P2', 'Auth / Frontend', { text: 'EN COURS — v1.1.0', bg: ORANGE, bold: true, color: '92400E' }],
  ], [1400, 1400, 3500, 3780]),
  pb(),
];
...
const sec4 = [
  ...
  tbl(['Champ', 'Valeur'], [
    ...
    ['Module concerné', 'Auth / Frontend (Axios interceptor)'],
    ['Priorité', { text: 'P2 — Majeur', bg: ORANGE, bold: true }],
    ['Statut', { text: 'EN COURS — correctif planifié v1.1.0', bg: ORANGE }],
    ['Assigné à', 'Rawan Atwe'],
    ['GitHub Issue', '#12 — label: bug, priority: P2'],
  ], [3000, 7080]),
  sp(),
...
const sec5 = [
  ...
  sp(),
  h2('5.1 Solution technique'),
  p('Le correctif consiste à ajouter un intercepteur de réponse Axios global dans le fichier src/api/axiosInstance.ts du frontend :'),
  sp(),
  p('// frontend/src/api/axiosInstance.ts', { font: 'Courier New', color: MUTED }),
  p('axiosInstance.interceptors.response.use(', { font: 'Courier New', color: TEXT }),
  p('  response => response,', { font: 'Courier New', color: TEXT }),
  p('  error => {', { font: 'Courier New', color: TEXT }),
  p('    if (error.response?.status === 401) {', { font: 'Courier New', color: TEXT }),
  ...
  p('    return Promise.reject(error);', { font: 'Courier New', color: TEXT }),
  p('  }', { font: 'Courier New', color: TEXT }),
  p(');', { font: 'Courier New', color: TEXT }),
  sp(),
  p('Sur la page /login, un message contextuel est affiché si le paramètre reason=session-expired est présent dans l\'URL.'),
  sp(),
  h2('5.2 Plan de déploiement via CI/CD'),
  tbl(['Étape', 'Action', 'Outil'], [
    ['1', 'Développement du correctif sur branche fix/BUG-07-token-interceptor', 'Git'],
    ['2', 'Exécution locale des 33 tests unitaires — vérification aucune régression', 'Jest (npm test)'],
    ['3', 'Ajout test unitaire pour l\'intercepteur Axios (mock axios + spy localStorage)', 'Vitest / Jest'],
    ['4', 'Pull Request vers develop — revue de code', 'GitHub PR'],
    ['5', 'CI/CD automatique : backend-test + frontend-build + security-audit', 'GitHub Actions'],
    ['6', 'Merge si CI green → deploy production', 'GitHub Actions'],
    ['7', 'Rejouer scénario CR-AUTH-05 → vérifier PASS', 'Test manuel'],
    ['8', 'Clôture GitHub Issue #12 + mise à jour journal v1.1.0', 'GitHub + §7'],
  ], [600, 4600, 4880]),
  sp(),
  ...
  tbl(['Étape', 'Résultat attendu après correctif'], [
    ...
    ['Statut CR-AUTH-05', { text: 'PASS', bg: GREEN, bold: true, color: '2E7D32' }],
  ], [4000, 6080]),
  pb(),
];
...
const sec6 = [
  ...
  tbl(['ID', 'Recommandation', 'Priorité', 'Bénéfice attendu', 'Version cible'], [
    ['AME-01', 'Intercepteur Axios 401 → redirection automatique (correctif BUG-07)', { text: 'P2', bg: ORANGE }, 'Éliminer le seul scénario FAIL du cahier de recettes', 'v1.1.0'],
    ['AME-02', 'Tests e2e NestJS avec base MariaDB de test en CI', { text: 'P2', bg: ORANGE }, 'Prévenir les régressions sur les flux complets (inscription → programme → envoi)', 'v1.1.0'],
    ['AME-03', 'Intégration Sentry (monitoring erreurs production)', { text: 'P2', bg: ORANGE }, 'Détecter proactivement les erreurs 5xx sans consulter les logs manuellement', 'v1.1.0'],
    ['AME-04', 'Pagination cursor-based sur GET /exercises', { text: 'P3', bg: LIGHT }, 'Performances : actuellement 47 000+ exercices chargés sans pagination', 'v1.1.0'],
    ['AME-05', 'Messages d\'erreur explicites côté frontend', { text: 'P3', bg: LIGHT }, 'UX : afficher err.response.data.message au lieu d\'un message générique', 'v1.1.0'],
  ...
  tbl(['Indicateur', 'Objectif', 'Valeur mesurée', 'Statut'], [
    ['Couverture tests AuthService', '> 90%', '98.55% (statements)', { text: '✅ OK', bg: GREEN }],
    ['Couverture tests ProgramsService', '> 80%', '97.82% (statements)', { text: '✅ OK', bg: GREEN }],
    ['Couverture globale', '> 50%', '48.61%', { text: '⚠ Proche', bg: ORANGE }],
    ['Scénarios recette PASS', '100%', '19/20 (95%)', { text: '⚠ 1 FAIL', bg: ORANGE }],
    ['Vulnérabilités npm critiques', '0', '0', { text: '✅ OK', bg: GREEN }],
    ['Build frontend CI', '< 60s', '< 45s (estimé)', { text: '✅ OK', bg: GREEN }],
  ], [3000, 2000, 2500, 2580]),
  sp(),
...
const sec7 = [
  ...
  tbl(['Indicateur', 'Valeur'], [
    ...
    ['Scénarios recette', '19 PASS / 1 FAIL (BUG-07 — non bloquant)'],
    ['Vulnérabilités npm', '0 critique'],
  ], [3000, 7080]),
  sp(),
  h2('7.3 v1.1.0 — Planifiée Q4 2026'),
  p('Tag Git : v1.1.0 (planifiée) — Branche cible : develop', { bold: true }),
  sp(),
  tbl(['Évolution', 'Type', 'Compétence liée'], [
    ['Correctif BUG-07 — intercepteur Axios token expiré', 'Bug fix', 'C4.2.2'],
    ...
    ['Messages d\'erreur explicites frontend', 'UX fix', 'AME-05'],
  ], [3800, 1600, 4680]),
  pb(),
];
